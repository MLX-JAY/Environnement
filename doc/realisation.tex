...
Les choix de conception reposent sur des patrons de conception éprouvés issus de
la littérature sur le génie logiciel~\cite{gamma1994design, freeman2004head}. Ces
patrons ont été sélectionnés pour répondre aux contraintes d'extensibilité et de
maintenabilité identifiées dans la section~\ref{subsec:enjeux}.

\paragraph{Patron Fabrique (Factory).}
Le patron Fabrique est mis en œuvre par les classes \texttt{BiomeFactory} et
\texttt{EvenementFactory}~\cite{gamma1994design}. La \texttt{BiomeFactory}
centralise la création de tous les objets biomes, permettant d'instancier un biome
à partir de son type sans connaître la classe concrète concernée. La
\texttt{EvenementFactory}, quant à elle, va plus loin que la simple instanciation~:
elle crée chaque événement sous une \emph{formation spatiale} précise (unique,
ligne horizontale, ligne verticale, carré $2 \times 2$, carré $3 \times 3$, T, L,
diagonale, cercle ou croix), selon des probabilités configurées dans le module
\texttt{config}. Cette capacité à construire des formations cohérentes apporte une
grande cohérence visuelle au projet, en donnant à chaque événement une empreinte
reconnaissable sur la grille. Ce patron garantit de plus que l'ajout d'un nouveau
type de biome ou d'événement ne nécessite de modifier que la fabrique et la classe
concrète correspondante, sans toucher au reste du code.

\paragraph{Patron Visiteur (Visitor).}
Le patron Visiteur est utilisé pour la génération d'événements liés aux biomes
via la classe \texttt{GenerateurEvenementVisitor}. Ce patron sépare l'algorithme
de génération des structures de données qu'il parcourt~\cite{gamma1994design}.
Ainsi, chaque type de biome peut être visité pour déclencher des événements
spécifiques à ses caractéristiques (par exemple, une Mer génère des événements
de type Pluie) sans modifier la hiérarchie des biomes. Dans notre code, cette
classe parcourt la carte à chaque round et applique la logique de génération
propre à chaque biome rencontré, ce qui maintient la responsabilité de génération
hors de la classe \texttt{Carte} elle-même.

\paragraph{Programmation par interfaces.}
L'interface \texttt{RegleTransformation} est implémentée par chacune des six règles
de transformation. Ce mécanisme garantit que tout objet implémentant cette interface
peut être évalué par le moteur de simulation de manière uniforme~: le
\texttt{ManageurBasique} itère simplement sur une liste de \texttt{RegleTransformation}
sans connaître les classes concrètes. Cela permet d'ajouter ou de supprimer une
règle sans modifier l'algorithme principal du moteur.
...
\begin{table}[H]
  \centering
  \caption{Valeurs initiales des propriétés des biomes (définies dans
           \texttt{ConfigurationBiome})}
  \label{tab:biomes_init}
  \begin{tabular}{lrrrr}
    \toprule
    \textbf{Biome} & \textbf{Température} & \textbf{Humidité}
                   & \textbf{Pollution} & \textbf{Purification} \\
    \midrule
    Forêt      & 18{,}0    & 70  &  5 & 20 \\
    Désert     & 45{,}0    &  5  &  0 &  2 \\
    Mer        & 15{,}0    & 100 & 10 &  5 \\
...
La deuxième formule décrit le \textbf{calcul des impacts sur les propriétés}.
...
La troisième formule assure le \textbf{bornage des valeurs}. Toutes les propriétés
