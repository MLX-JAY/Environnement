...
\begin{itemize}
  ...
  \item \textbf{Module \texttt{gui}}~: gère l'interface utilisateur. Il contient
    les classes d'affichage, à savoir \texttt{MainGUI}, \texttt{MainDisplayer},
    \texttt{PanelStatistique}, \texttt{PanelTemps}, \texttt{PanelEdition}
    et \texttt{TutorielPanel}, ainsi que la classe de rendu (\texttt{StrategiePeinture}).
...
\fig{images/architecture.png}{0.85\textwidth}{!}{Architecture modulaire de l'application Environnement}{architecture}
...
\begin{table}[htbp]
  \centering
  \caption{Valeurs initiales des propriétés des biomes (source~: \texttt{ConfigurationBiome})}
  \label{tab:biomes_init}
  \begin{tabular}{lrrrr}
    \toprule
    \textbf{Biome} & \textbf{Température} & \textbf{Humidité} & \textbf{Pollution} & \textbf{Purification} \\
    \midrule
    Forêt      & 50 & 70 &  5 & 20 \\
    Désert     & 80 & 10 & 10 &  0 \\
    Mer        & 40 & 90 &  0 & 10 \\
...
point d'entrée de l'application. Comme l'illustre la figure~\ref{fig:schema_edition},
elle permet à l'utilisateur de configurer la carte avant le lancement de la simulation. La zone supérieure affiche la grille
...
figure~\ref{fig:schema_simulation}, elle est composée de trois zones
fonctionnelles.

Les trois zones visibles dans la figure~\ref{fig:schema_simulation} sont les
suivantes~:
...
figure~\ref{fig:fenetre_simulation}.
...
\fig{images/mode_edition.png}{0.9\textwidth}{!}{Schéma conceptuel de l'interface de configuration initiale}{schema_edition}
\fig{images/mode_simulation.png}{0.85\textwidth}{!}{Schéma conceptuel de l'interface de pilotage dynamique}{schema_simulation}
\fig{images/fenetre_simulation.png}{0.9\textwidth}{!}{Fenêtre principale de simulation avec ses trois zones fonctionnelles}{fenetre_simulation}
\fig{images/fenetre_aide.png}{0.9\textwidth}{!}{Fenêtre de tutoriel}{fenetre_aide}
...
configurées dans \texttt{Configuration\allowbreak Creation\allowbreak Evenement}. Le patron Visiteur est
