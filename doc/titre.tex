% ============================================================
%  PAGE DE GARDE — titre.tex
% ============================================================
\begin{titlepage}
  \centering

  % --- Bandeau supérieur ---
  \begin{minipage}[t]{0.45\textwidth}
    \raggedright
    \includegraphics[height=2cm]{images/logo_universite.png}
  \end{minipage}
  \hfill
  \begin{minipage}[t]{0.45\textwidth}
    \raggedleft
    \textsc{\small CY Cergy Paris Université}\\[0.2em]
    \small UFR Sciences et Techniques\\
    \small Département Informatique
  \end{minipage}

  \vspace{0.5cm}
  \hrule height 0.8pt
  \vspace{0.5cm}

  % --- Mention du cursus ---
  {\large\textsc{Licence 2 Informatique}}\\[0.3em]
  {\normalsize Génie Logiciel et Conception de Logiciels}\\[0.5em]
  {\large\bfseries RAPPORT DE PROJET}

  \vspace{1.5cm}

  % --- Image décorative ---
  \begin{center}
    \fbox{%
      \begin{minipage}{0.65\textwidth}
        \centering
        \vspace{0.3cm}
        % Remplacer par : \includegraphics[width=\textwidth]{images/logo.png}
        \includegraphics[width=\textwidth]{images/logo.png}
        \vspace{0.3cm}
      \end{minipage}%
    }
  \end{center}

  \vspace{1.2cm}

  % --- Titre du projet ---
  {\Huge\bfseries Simulateur d'Écosystème}\\[0.5em]
  {\LARGE\bfseries Dynamique en Java}

  \vspace{0.4cm}
  \hrule height 0.4pt
  \vspace{0.4cm}

  {\large\itshape Conception et implémentation d'un simulateur}\\
  {\large\itshape d'évolution de biomes géré par événements}

  \vspace{2cm}

  % --- Auteurs ---
  \begin{minipage}{0.6\textwidth}
    \centering
    \large
    \begin{tabular}{l}
      \textsc{Feraoun} Mohamed Amine\\[0.4em]
      \textsc{Anurajan} Thenuxshan\\[0.4em]
      \textsc{Ouardani} Nidhal
    \end{tabular}
  \end{minipage}

  \vfill

  % --- Encadrant et date ---
  \begin{minipage}{0.48\textwidth}
    \raggedright
    \textbf{Encadrant~:}\\[0.3em]
    \underline{\hspace{5cm}}\\
    {\small (Nom du professeur encadrant)}
  \end{minipage}
  \hfill
  \begin{minipage}{0.48\textwidth}
    \raggedleft
    \textbf{Année universitaire~:}\\[0.3em]
    2025 -- 2026\\[0.5em]
    \textbf{Date de dépôt~:} Avril 2026
  \end{minipage}

  \vspace{0.8cm}
  \hrule height 0.8pt

\end{titlepage}
